...
def pie_graphs(inp_df: pl.DataFrame):
    ...
    two_in_one.update_traces(textfont_size=20,
                             marker={'colors': COLOR5,
                                     'line': {'color': '#000000',
                                              'width': 2}
                                     },
                             )
    two_in_one.update_layout(
                title_text='Разбивка всех вакансий по грейдам'
                           ' и требуемому опыту',
                title_x=0.5,
                showlegend=False,
                font={'size': 18},
                margin={'t': 70, 'b': 70, 'l': 0, 'r': 0},
...
def violin_salary(inp_df: pl.DataFrame):
    '''
    Построение violin plot предлагаемых зарплат по грейдами
    args
        pl.DataFrame - исходный dataframe для построения графика
    return
    ...
    fig.update_layout(
                title_text='Разбивка предлагаемых зарплат по грейдам',
                title_x=0.5,
                # showlegend=True,
                font={'size': 18},
                margin={'t': 70, 'b': 70, 'l': 0, 'r': 0},
...
layout = html.Div([
    ...
    html.P('Отображать количество вакансий по периоду:'),
    ...
    html.P('Отображать количество вакансий по грейду:'),
...
def scatter_cnt_vacancies(by_period: str, by_grade: str):
    ...
    if by_grade == 'Суммарно':
        ttl = f'Количество вакансий по {ttl_incert}'
        ...
        ttl = f'Количество вакансий по {ttl_incert} и грейдам'
        fig = go.Figure()
